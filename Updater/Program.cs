...
namespace Updater
{
    class Program
    {
        static void Main(string[] args)
        {
            if (args.Length < 2)
            {
                Console.WriteLine("Missing install folder argument! Press a key to exit the OST updater.");
            ...
            {
                Console.WriteLine("Missing -installfolder <folder>! Press a key to exit the OST updater.");
                Console.ReadKey(true);
                return;
            }
            ...
            {
                Console.WriteLine("Either update folder or installation folder is invalid! Press a key to exit the OST updater.");
                Console.ReadKey(true);
                return;
            }

            // Kill running OST before continuing (but allow 1 second for OST to shut itself down).
            Console.WriteLine("Updating Overload Server Tool - please wait!");
            Console.WriteLine();
            ...
            try
            {
                ...
                // At this point OST should already have saved settings and shut itself down.
                // But we make sure it isn't running just to be on the safe side.
                KillRunningProcess("OverloadServerTool");
            ...
            catch (Exception ex)
            {
                ...
                Console.WriteLine();
                Console.WriteLine($"Press a key to exit the OST updater.");
                Console.ReadKey(true);
                return;
            }

            // Launch updated OST application.
            try
            {
                Process appStart = new Process();
                appStart.StartInfo.FileName = Path.Combine(installFolder, "OverloadServerTool.exe");
