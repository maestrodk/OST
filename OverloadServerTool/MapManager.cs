...
namespace OverloadServerTool
{
    ...
    public class OverloadMapManager
    {
        ...
        // Note! To ensure compability with OCT both OCT and OST uses the same hidden marker!
        ...
        public bool UpdateAllMaps(string mapListUrl, string dlcMaps, string applicationDataMaps)
        {
            ...
            finally
            {
                foreach (KeyValuePair<string, OverloadMap> sortedMap in Maps)
                {
                    //if (!OverloadServerApplication.ValidFileName(sortedMap.Value.LocalZipFileName, true)) sortedMap.Value.LocalZipFileName = null;
                    //if (!OverloadServerApplication.ValidFileName(sortedMap.Value.LocalDLCZipFileName, true)) sortedMap.Value.LocalDLCZipFileName = null;
                }
            }
