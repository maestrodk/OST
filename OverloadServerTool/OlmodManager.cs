...
namespace OverloadServerTool
{
    public class OlmodManager
    {
        ...
        public void DownloadAndInstallOlmod(OlmodRelease release, string localInstallFolder)
        {
            ...
            {
                if (OverloadServerToolApplication.ValidFileName(localTempZip, true)) try { File.Delete(localTempZip); } catch { }
                if (OverloadServerToolApplication.ValidDirectoryName(localTempFolder, true)) try {OverloadServerToolApplication.RemoveDirectory(localTempFolder); } catch { }
            }
        }
    }
}
