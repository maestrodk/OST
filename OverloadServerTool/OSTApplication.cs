...
{
    static class OverloadServerToolApplication
    {
        /// <summary>
        ...
        static void Main(string[] args)
        {
            ...
            string debugFileFolder = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "OverloadServerTool\\Debug");
            string debugFileName = Path.Combine(debugFileFolder, String.Format($"OST_Debug_{startDateTime.Replace(":", "").Replace("-", "").Replace(" ", "_")}.txt"));
            try { Directory.CreateDirectory(debugFileFolder); } catch { }

            LogDebugMessage("OST application startup.", debugFileName);
            ...
            try
            {
                LogDebugMessage("Starting OST main UI thread.", debugFileName);
                Application.Run(new OSTMain(args, debugFileName));
                LogDebugMessage("OST main exit - shutting UI thread.", debugFileName);
            }
            catch (Exception ex)
            {
                LogDebugMessage(String.Format($"OST exited due to an unexpected error: {ex.Message} at {ex.TargetSite}"), debugFileName);
            }
        }
        ...
        public static void LogDebugMessage(string message, string logFileName = null)
        {
            ...
            {
                string debugFileFolder = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "OverloadServerTool\\Debug");
                string debugFileName = Path.Combine(debugFileFolder, String.Format($"OS_Debug.txt"));
                try { Directory.CreateDirectory(debugFileFolder); } catch { }
        ...
        public static void RemoveInstallDirectory(string path, string debugFileName)
        {
            ...
            try
            {
                while (Directory.GetParent(path).Name.ToLower().Contains("ost_update")) path = Directory.GetParent(path).FullName;
